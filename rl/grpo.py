...
def grpo_param_map(gcfg: dict, *, lr: float, accum: int, group: int, temperature: float, top_p: float, max_new: int) -> Dict[str, float]:
    ...
    return {
        ...
        "w_cov": float(rew.get("w_cov", 0.40)),
        "w_dense_c": float(rew.get("w_dense_c", 0.60)),
        "dense_w_dist": float(rew.get("dense_w_dist", 0.60)),
        "dense_w_area": float(rew.get("dense_w_area", 0.40)),
        "w_dir": float(rew.get("w_dir", 0.70)),
        "w_progress": float(rew.get("w_progress", 0.30)),
        "w_dense_f": float(rew.get("w_dense_f", 0.80)),
        "w_edge": float(rew.get("w_edge", 0.20)),
