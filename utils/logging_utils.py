...
def inject_visual_dims_into_logs(logs: Dict[str, Any], model_ref: Any) -> None:
    """Kept as a no-op hook; old DINO/CLIP dim logging was removed."""
    return
...
def format_train_log_line(
    ...
    *,
    ...
    """与 EnhancedLoggingCallback 终端行一致的一行字符串。"""
...
class EnhancedLoggingCallback(TrainerCallback):
    def __init__(self, output_dir: str, writer: Optional[SummaryWriter] = None):
        self.log_dir = os.path.join(output_dir, "tb")
        self.is_world_process_zero = _is_main_process()
        self._owns_writer = False
        self.writer = writer
        if self.is_world_process_zero and self.writer is None:
    ...
    def on_log(self, args, state: TrainerState, control: TrainerControl, logs=None, model=None, **kwargs):
        ...
        if trainer_reported_loss is not None:
            self.writer.add_scalar("train/loss", float(trainer_reported_loss), step)
        if total_loss is not None:
            self.writer.add_scalar("train/loss_total", float(total_loss), step)
        if lm_loss is not None:
    ...
    def __del__(self):
        if getattr(self, "_owns_writer", False) and getattr(self, "writer", None) is not None:
            self.writer.close()
