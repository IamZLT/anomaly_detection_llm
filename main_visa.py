...
def main(args):
    ...
    # 🔧 设置投影层为微调模式（极小学习率）
    logger.info("🔧 Setting projection layers to fine-tuning mode (very small learning rate)")
    image_proj.train()  # 设置为训练模式
    patch_proj.train()  # 设置为训练模式
    for param in image_proj.parameters():
        param.requires_grad = True  # 允许梯度更新
    for param in patch_proj.parameters():
        param.requires_grad = True  # 允许梯度更新
    print("🔧 Projection layers set to fine-tuning mode - training with very small learning rate")
    ...
    # 优化器：为不同模块设置不同学习率
    logger.info("📊 Optimizer: Fine-tuning projection layers with very small LR + training soft_prompt")
    
    # 为投影层设置极小的学习率（微调）
    proj_lr = args.learning_rate * args.proj_finetune_ratio  # 默认是主学习率的1/100
    
    optimizer = optim.Adam([
        {'params': image_proj.parameters(), 'lr': proj_lr, 'weight_decay': args.weight_decay},
        {'params': patch_proj.parameters(), 'lr': proj_lr, 'weight_decay': args.weight_decay},
        {'params': soft_prompt.parameters(), 'lr': args.learning_rate, 'weight_decay': args.weight_decay}
    ])
    
    print(f"📊 Learning rates:")
    print(f"   - Image projection: {proj_lr:.2e}")
    print(f"   - Patch projection: {proj_lr:.2e}")
        ...
        for obj in tqdm(obj_list, desc="Training Objects", position=1, leave=False):
            ...
            for i in tqdm(range(shot), desc=f"  Training Shots ({obj})", position=2, leave=False):
                ...
                torch.nn.utils.clip_grad_norm_(
                    list(image_proj.parameters()) + list(patch_proj.parameters()) + list(soft_prompt.parameters()), 
...
if __name__ == '__main__':
    ...
    # paths
    parser.add_argument("--data_path", type=str, default="/data2/zlt/code/abnormal_dataset/visa_save/1cls/", help="path to test dataset")
    parser.add_argument("--save_path", type=str, default='./results/main_visa_shot2_retrain', help='path to save results and checkpoints')
    ...
    parser.add_argument("--pretrained_proj_path",default='model_card/pretrain_mvtec_for_visa/projection_layers_epoch_5.pth', type=str, help='path to pretrained projection layers checkpoint (e.g., ./checkpoints/projection_layers/best_projection_layers.pth)')
    
    # training hyperparameters
    parser.add_argument("--epochs", type=int, default=20, help="number of training epochs")
    parser.add_argument("--test_interval", type=int, default=5, help="test every N epochs (default: 5)")
    ...
    # model
    parser.add_argument("--dataset", type=str, default='visa', help='dataset name (mvtec or visa)')
    # best: 12, 16, 20, 24
